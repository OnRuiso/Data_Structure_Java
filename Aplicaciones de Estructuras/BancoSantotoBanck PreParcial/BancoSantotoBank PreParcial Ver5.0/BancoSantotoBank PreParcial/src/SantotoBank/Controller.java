...
public class Controller
{
	...
	public int getFlagRegistro() {		return flagRegistro;	}
	public void setFlagRegistro(int flagRegistro) {		this.flagRegistro = flagRegistro;	}

	//#----------------------------------------------------------------------------------------------------------------------------------------

	int flag = 0;//auxiliar para registro
	...
	public void Launch()
	{
		...
		while(estado)
		{
				...
				{JOptionPane.showInternalMessageDialog(null, "Error en el Ingreso de datos, vulvelo a intentar "+"["+ e.getMessage() +"]");	}
			}
	}
	...
	{
		flag = flag + 1;
		setFlagRegistro(flag);
		System.out.println(getFlagRegistro());
		
		if(getFlagRegistro() == 6) { JOptionPane.showMessageDialog(null, "Numero Maximo de usuarios ocupados, Si quiere \n modifique un Slot en la opcion determinada por el sistema \n '[3] Modificar una cuenta de Usuario"); }
		else
		{
			int aux = 0;
	...
	public void List()
	{
		for(int i=0 ; i < clienteInfo.length ; i++)
		{
			JOptionPane.showMessageDialog(null, "ID: "+ clienteInfo[i].getNumero() +"\n"
											...
											+ "Cuenta "+clienteInfo[i].getNumeroCuenta()[0]+": " + clienteInfo[i].getTipo()[0]  +"__"+ clienteInfo[i].getSaldo()[0] + "\n"
											+ "Cuenta "+clienteInfo[i].getNumeroCuenta()[1]+": " + clienteInfo[i].getTipo()[1]  +"__"+ clienteInfo[i].getSaldo()[1] + "\n"
											+ "Cuenta "+clienteInfo[i].getNumeroCuenta()[2]+": " + clienteInfo[i].getTipo()[2]  +"__"+ clienteInfo[i].getSaldo()[2] + "\n");
		}
	}
	//#----------------------------------------------------------------------------------------------------------------------------------------
	...
	//------------------------------------------------------------------------------------------------------------------------------------------------
	public void burbujaBombonBellota()
	{		
		for (int i=0 ; i < clienteInfo.length - 1 ; i++ )
		{
			for (int j = i+1 ; j < clienteInfo.length ; j++)
			{
				if( clienteInfo[i].getApellido().compareTo(clienteInfo[j].getApellido()) > 0 )
				{
					Usuario a = clienteInfo[i];
										
					clienteInfo[i]= clienteInfo[j];
					clienteInfo[j] = a;
				}
			}
		}
