...
{	
	...
	public void especDeleteCuenta()
	{
		...
		if(user_banco.containsKey(id_cedula) && user_cuenta.containsKey(id_cedula))
		{
			...
			int clienteNum = (int) clientePos - 1; 
			System.out.println("La posicion que se eleminara es: "+clienteNum);
				
			Cliente aux = gente.remove(clienteNum); //como no son posiciones en una tabla Hash si no elementos , ademas de remover de la lista , le digo que busque el elemento para remover
			Cuenta aux1 = cuenta.remove(clienteNum);
			
			clientes_banco.remove(aux); //no utilizo poll porque eliminaria el que este en la cola
			clientes_cuenta.remove(aux1);
